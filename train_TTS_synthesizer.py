...
import numpy as np
import utils
from data_utils import TextAudioLoader_TTS_synth, TextAudioCollate_TTS_synth, DistributedBucketSampler
...
def run(rank, n_gpus, hps):
  ...
  # データセットの読み込み君 
  train_dataset = TextAudioLoader_TTS_synth(audiopaths_and_text= hps["train_data_path"] ,
                                  hparams            = hps)
  ...
  collate_fn = TextAudioCollate_TTS_synth(hparams=hps)
  ...
  # 1番目のGPUであれば
  if rank == 0:
    eval_dataset = TextAudioLoader_TTS_synth(audiopaths_and_text=hps["eval_data_path"], 
                                   hparams            =hps)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (wav,\
    ...
    with autocast(enabled=hps["fp16_run"]):
      ...
      with autocast(enabled=False):
        loss_length_l2 = torch.sum(l2_dur_loss).float()
...
def evaluate(hps, epoch, nets, eval_loader, writer_eval):
    ...
    f0_lengths             = torch.tensor(f0.size(2), dtype=torch.int64).view(-1).cuda(rank)
