...
import utils
from data_utils import TextAudioLoader_SVS_f0diff, TextAudioCollate_SVS_f0diff, DistributedBucketSampler
...
def run(rank, n_gpus, hps):
  ...
  # データセットの読み込み君 
  train_dataset = TextAudioLoader_SVS_f0diff(audiopaths_and_text= hps["train_data_path"] ,
                                  hparams            = hps)
  ...
  collate_fn = TextAudioCollate_SVS_f0diff()
  ...
  # 1番目のGPUであれば
  if rank == 0:
    eval_dataset = TextAudioLoader_SVS_f0diff(audiopaths_and_text=hps["eval_data_path"], 
                                              hparams            =hps)
  ...
  for batch_idx, (f0,f0_lengths,\
    ...
    with autocast(enabled=hps["fp16_run"]):
      loss_f0 = net_g(f0=f0.cuda(rank),  
                      ...
                      NoteIDs=noteID.cuda(rank),
                      NoteID_len=noteID_lengths.cuda(rank),
...
def evaluate(hps, net_g, eval_loader, writer_eval):
    ...
    NoteIDs=noteID            [0]   .view(1,-1)   .cuda(rank)   
    NoteID_len=noteID_lengths [0]   .view(-1)     .cuda(rank)     
